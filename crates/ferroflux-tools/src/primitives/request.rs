...
pub fn check_ssrf(url: &str) -> Result<()> {
    ...
    // SSRF Vulnerability: Resolve IPs and block internal/private network ranges to prevent SSRF bypasses via DNS rebinding or obfuscation
    if let Err(e) = ferroflux_security::network::validate_url(url) {
        anyhow::bail!("SSRF Protection blocked request: {}", e);
    }
