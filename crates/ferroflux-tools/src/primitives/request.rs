...
pub fn check_ssrf(url: &str) -> Result<()> {
    ...
    // Use robust SSRF check that resolves IPs rather than naive string matching
    ferroflux_security::network::validate_url(url).map_err(|e| anyhow::anyhow!("SSRF check failed: {}", e))?;
