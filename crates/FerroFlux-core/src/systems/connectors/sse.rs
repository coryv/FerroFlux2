use crate::components::connectors::SseTriggerConfig;
use crate::components::NodeConfig;
use crate::resources::sse_registry::{SseConnectionHandle, SseTriggerRegistry};
use crate::resources::TokioRuntime;
use crate::store::BlobStore;
use crate::traits::trigger::{TriggerEvent, TriggerSender};
                        ...
                        match item {
                            Ok(bytes) => {
                                if let Ok(s) = std::str::from_utf8(&bytes) {
                                    buffer.push_str(s);
                                    process_sse_buffer(&mut buffer, &node_id, &sender, &blob_store).await;
                                }
                            }
                            Err(e) => {
