...
#
# Most importantly: catches {{ settings.x }} and {{ inputs.x }} without 'get',
# which the Rust validator does NOT detect (they're silent runtime bugs).
#
# Usage:
...
check_file() {
  ...
  # ── Check 1: {{ settings.x }} without get ─────────────────────────────────
  # The Rust validator does NOT catch this. It's a silent runtime bug.
  # Correct: {{ get 'settings.field' }}
  # Wrong:   {{ settings.field }}       ← will silently produce wrong output
  #
  # The bare form "{{ settings.x }}" works ONLY as an isolated value, but the
  # skill convention is ALWAYS to use get, so we flag all bare accesses.
  while IFS= read -r line; do
    # Skip comment lines
    [[ "$line" =~ ^[[:space:]]*# ]] && continue
    # Skip if the {{ }} already contains get '
    if echo "$line" | grep -qE "\{\{[^}]*get\s+'[^']*settings\."; then continue; fi
    emit_error "bare-settings-access" "$(echo "$line" | xargs) — use {{ get 'settings.field' }}"
  done < <(grep -nE '\{\{[^}]*settings\.' "$file" 2>/dev/null || true)

  # ── Check 2: {{ inputs.x }} without get ───────────────────────────────────
  # Same as above but for inputs. Bare {{ inputs.x }} in string interpolation
  # will not resolve correctly. Correct: {{ get 'inputs.field' }}
  while IFS= read -r line; do
    [[ "$line" =~ ^[[:space:]]*# ]] && continue
    if echo "$line" | grep -qE "\{\{[^}]*get\s+'[^']*inputs\."; then continue; fi
    emit_error "bare-inputs-access" "$(echo "$line" | xargs) — use {{ get 'inputs.field' }}"
  done < <(grep -nE '\{\{[^}]*inputs\.' "$file" 2>/dev/null || true)

  # ── Check 3: http_client step without returns block ───────────────────────
  ...
  # ── Check 7: platform.api_key / platform.token access ─────────────────────
  # These don't exist on the platform object — only platform.base_url and
  # platform.headers are valid. This is a hard runtime error.
  while IFS= read -r line; do
    [[ "$line" =~ ^[[:space:]]*# ]] && continue
    emit_error "invalid-platform-property" "$(echo "$line" | xargs) — only platform.base_url and platform.headers are valid; credentials are in platform.headers"
  done < <(grep -nE '\{\{[^}]*platform\.(api_key|token|secret|key|credentials)' "$file" 2>/dev/null || true)
